...
function apa_epi_f_generate_output(){
	...
	while ( $posts_query->have_posts() ):
		...
		$html['year'][$counter] .= get_the_date('Y') .$line_break;
		$counter++;
	endwhile;
	$counter = 0;
	while ( $posts_query->have_posts() ):
	...
	if ($count > 0) {
			...
			for( $i = 0; $i < $count; $i++ ){
				$data = array(
					($html['year']) ? $html['year'][$i] : "",
					($html['date']) ? $html['date'][$i] : "",
					($html['title']) ? $html['title'][$i] : "",
					($html['words']) ? $html['words'][$i] : "",
					($html['url']) ? $html['url'][$i] : "",
					($html['category']) ? $html['category'][$i] : ""
				);
			...
			fclose($myfile);
?>
	<div class='updated'><strong><?php echo $count . ' </strong>'; _e( 'posts exported successfully!', 'export-post-info' ); ?> · <a href="<?php $upload_dir = wp_upload_dir(); echo $upload_dir['baseurl'] . '/export-post-info-' . get_option( 'epi_random_string_filename' ) . '.csv'; ?>"><strong><?php _e( 'Click here', 'export-post-info' ); ?></strong></a> <?php _e( 'to download the file', 'export-post-info' ); ?>.</div>
<?php
	} else {
	?>
	<div class='updated'><span style="color:red"><strong><?php _e( 'No data to export available!', 'export-post-info' ); ?></strong></span> <?php _e( 'Your WordPress has', 'export-post-info' ); ?> <?php $count_posts = wp_count_posts(); $published_posts = $count_posts->publish; echo $published_posts ?> <?php _e( 'published posts', 'export-post-info' ); ?>. <?php _e( 'Check your posts <a href="edit.php">here</a>', 'export-post-info' ); ?>.</div>
